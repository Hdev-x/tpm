...
function switchOrderTab(side) {
    ...
    if (isBuy) {
        ...
        const holding = holdingsData.find(h => h.coinCode === window.currentSymbol);
        const qty = holding ? holding.coinCount : 0;
        const ticker = (window.currentSymbol || '').replace(/USDT$/, '').replace('_SPBL', '');
...
async function loadWallet() {
    ...
    const res = await fetch('/coin/wallet').then(r => r.json());
...
async function loadPendingOrders() {
    ...
    const res = await fetch('/coin/pending').then(r => r.json());
    pendingOrders = res || [];
    renderPendingOrders();
...
function checkPendingOrders(price) {
    ...
    Promise.all(toExecute.map(o =>
        fetch('/coin/executePending', {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            body: new URLSearchParams({ orderNo: o.orderNo, coinCode: o.coinCode, orderType: o.orderType, orderPrice: price, orderCount: o.orderCount })
...
async function cancelPendingOrder(orderNo) {
    ...
    const res = await fetch('/coin/cancelOrder', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: new URLSearchParams({ orderNo })
    }).then(r => r.text());
