...
/* ====================================================
   실시간 댓글 뷰어 (STOCK)
   ==================================================== */
let stockChatClient = null;
let stockChatSub = null;
...
function connectStockChat(code) {
    ...
    if (stockChatClient && stockChatClient.connected) {
        if (stockChatSub) stockChatSub.unsubscribe();
        stockChatSub = stockChatClient.subscribe('/topic/market/STOCK/' + code, msg => {
    ...
    const socket = new SockJS('/ws-stock');
    stockChatClient = Stomp.over(socket);
    stockChatClient.debug = null;
    stockChatClient.connect({}, () => {
        if (stockChatSub) stockChatSub.unsubscribe();
        stockChatSub = stockChatClient.subscribe('/topic/market/STOCK/' + currentChatCode, msg => {
            const dto = JSON.parse(msg.body);
            if (dto.type === 'DELETE') {
                const el = document.querySelector('.chat-msg[data-no="' + dto.commentNo + '"]');
                if (el) el.remove();
